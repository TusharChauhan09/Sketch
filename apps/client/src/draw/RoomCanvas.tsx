...
import { Circle, LineSquiggle, Pencil, Square } from "lucide-react";
...
export default function RoomCanvas({ roomId }: { roomId: string }) {
  ...
  useEffect(() => {
    const ws = new WebSocket(
      `${WS_BACKEND_URL}?token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiJmYzZkODQ5Ni01ZjRjLTQ3M2UtYTU1OS00NTM0YjQ5NmYyMDAiLCJpYXQiOjE3NjI5NDkzNTQsImV4cCI6MTc2Mjk3ODE1NH0.01zKPLuwX5lXvEH78ArLhWvhvukpNLovyUiWTfcRH6A`
    );
    ws.onopen = () => {
    ...
    <div className="relative min-w-screen h-screen">
      ...
      >
        <div className=" w-60 min-h-100 flex flex-col border-0 rounded-lg bg-neutral-700">
          ...
          >
            <div>current choice: {shapeChoice}</div>
            <div className="w-full h-full bg-neutral-500 border-2 pt-1 rounded-2xl flex items-center justify-around m-[0.25px]">
              <div>
                <button
                  className="text-4xl hover:cursor-pointer hover:bg-neutral-400  rounded-lg"
              ...
              <div>
                <button
                  className="text-4xl hover:cursor-pointer hover:bg-neutral-400rounded-lg text-white"
